...
document.addEventListener('click', (event) => {
    ...
    if (navBar.contains(event.target)) {
        ...
        if (event.target.dataset.action === 'login') {
            ...
            document.getElementById('login-loader')?.classList.add('show');
            ...
            let p = fetch(url).then((resp) => resp.json())
                ...
                .catch((err) => {
                    showNotificationBar('danger', 'Error', 'Something went wrong');
            ...
            if (DataValidation(nm, em, pw)) {
                ...
                document.getElementById('signup-loader').classList.add('show');
                ...
                fetch(url, {
                    ...
                    .finally(() => {
                        ...
                        document.getElementById('signup-loader').classList.remove('show');
                    })
            }
        }
        ...
        if (['wishlist', 'watched', 'favorites'].includes(action)) {
            if (loginStatus.found) {
                ...
                const actionBtn = ele.querySelector('.' + action + '-btn');

                actionBtn.disabled = true;
                actionBtn.classList.add('show-loader');
                ...
                let p = fetch(url, {
                    ...
                    .then((data) => {
                        ...
                        showNotificationBar('success', 'Added', `${ele.children[1].children[0].innerText} is added to ${action}`);
                    })
                    .finally(() => {
                        actionBtn.disabled = false;
                        actionBtn.classList.remove('show-loader');
                    })
            } else {
                showNotificationBar('alert', 'Login', 'Please Login in order to add to ' + action);
                openLogin();
            }
        }
    }
